...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 2. No wavelength provided, expected empty array of tth
        #    and wavelength UserWarning
        ...
        (  # 2. Wavelength provided, expect tth values of 2*arcsin(q) in degree
            4 * np.pi,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # C1: Invalid q values that result in tth > 180 degrees, expect
            #     ValueError
            4 * np.pi,
            np.array([0.2, 0.4, 0.6, 0.8, 1, 1.2]),
            ValueError,
        ),
        (  # C2: Wrong wavelength that results in tth > 180 degrees, expect
            #     ValueError
            100,
            np.array([0, 0.2, 0.4, 0.6, 0.8, 1]),
            ValueError,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # 2. Wavelength provided, expect expected q values are sin15, sin30
            #    , sin45, sin60, sin90
            4 * np.pi,
...
@pytest.mark.parametrize(
    ...
    [
        # C1: Invalid tth value of > 180 degrees provided, with or without
        #     wavelength
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # 2. Valid q values containing 0, expect d values with divide by
            #    zero warning
            np.array(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # C3: User specified valid tth values between 0-180 degrees (without
        #     wavelength)
        (
            None,
            np.array([0, 30, 60, 90, 120, 180]),
        ...
        (  # C4: User specified valid tth values between 0-180 degrees (with
            #     wavelength)
            4 * np.pi,
            np.array([0, 30.0, 60.0, 90.0, 120.0, 180.0]),
...
@pytest.mark.parametrize(
    "wavelength, tth, expected_error_type, expected_error_msg",
    [
        (  # C1: Invalid tth value of > 180 degrees, no wavelength, expect two
            #     theta ValueError
            None,
            np.array([0, 30, 60, 90, 120, 181]),
            ValueError,
        ...
        (  # C2: Invalid tth value of > 180 degrees with wavelength, expect
            #     two theta ValueError
            4 * np.pi,
            np.array([0, 30, 60, 90, 120, 181]),
            ValueError,
            (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # C3: Valid d values, no wavelength, expect valid and non-empty tth
        #     values
        (
            None,
        ...
        (  # C4: Valid d values with wavelength, expect valid and non-empty
            #     thh values
            4 * np.pi,
            np.array(
...
@pytest.mark.parametrize(
    ...
    [
        # C1: Invalid d values that result in tth > 180 degrees, expect
        #     invalid q, d, or wavelength ValueError
        (4 * np.pi, np.array([1.2, 1, 0.8, 0.6, 0.4, 0.2]), ValueError),
        # C2: Wrong wavelength that result in tth > 180 degreesm, expect
        #     invalid q, d, or wavelength ValueError
