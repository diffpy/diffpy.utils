...
@pytest.mark.parametrize(
    ...
    [  # config file in home is present, no config in cwd.  various runtime
        ...
        # C3: just owner name passed in at runtime.
        # expect runtime_oname but others from config
        (
        ...
        # C4: just owner email passed in at runtime.
        # expect runtime_email but others from config
        (
...
@pytest.mark.parametrize(
    ...
    [  # tests as before but now config file present in cwd and home but orcid
        ...
        # C4: just owner email passed in at runtime.  expect runtime_email but
        # others from config
        (
            {"owner_email": "runtime@email.com"},
            {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # C1: Both mass density and packing fraction are provided, expect
        ...
        (  # C2: None of mass density or packing fraction are provided, expect
            # ValueError exception
            {
                "sample_composition": "SiO2",
                "energy": 10,
...
def test_compute_mu_using_xraydb_bad(inputs):
    with pytest.raises(
        ...
        match=(
            "You must specify "
            "either sample_mass_density or packing_fraction, but not both. "
