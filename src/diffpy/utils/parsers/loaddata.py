...
def loadData(
    ...
    The data block is identified as the first matrix block of at least minrows
    rows and constant number of columns. This seems to work for most of the
    datafiles including those generated by diffpy programs.
    ...
    minrows: int
        Minimum number of rows in the first data block. All rows must have the
        same number of floating point values.
    headers: bool
    ...
    hignore: list
        ...
        any elements in hignore. e.g. hignore=['# ', '['] causes the following
        lines to be skipped: '# qmax=10', '[defaults]'.
    kwargs:
    ...
    delimiter: str
        Delimiter for the data in the block (default use whitespace). For comma
        -separated data blocks, set delimiter to ','.
    unpack: bool
        Return data as a sequence of columns that allows tuple unpacking such
        as x, y = loadData(FILENAME, unpack=True). Note transposing the loaded
        array as loadData(FILENAME).T has the same effect.
    usecols:
    ...
    if not os.path.exists(filename):
        raise IOError(
            ...
            f"Please rerun the program specifying a valid filename."
        )
